...
def orca(graph):
    path = '/u/home/r/rlwillia/google-research/bigg/bigg/evaluation/orca/tmp.txt'
    f = open(path, 'w')
    ...
    output = sp.check_output(['/u/home/r/rlwillia/google-research/bigg/bigg/evaluation/orca/orca', 'node', '4', path, 'std'])
...
def mmd_weights_only(sample_graphs, target_graphs, kernel):
  ...
  for i in range(len(sample_list)):
      hist_temp, _ = np.histogram(sample_list[i], range = (-1e-5, max_), bins=200, density=False)
      hist_temp = hist_temp / hist_temp.sum()
      sample_ref.append(hist_temp)
  
  for i in range(len(target_list)):
      hist_temp, _ = np.histogram(target_list[i], range = (-1e-5, max_), bins=200, density=False)
      hist_temp = hist_temp / hist_temp.sum()
