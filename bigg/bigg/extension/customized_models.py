...
class BiggWithEdgeLen(RecurTreeGen):
    ...
    def embed_edge_feats(self, edge_feats):
        ...
        out = edge_embed.reshape(edge_feats.shape[1], self.num_layers, self.embed_dim).movedim(0, 1)
        out_h = torch.cat([out, self.leaf_h0], dim = 0)
        out_c = torch.cat([out, self.leaf_c0], dim = 0)
